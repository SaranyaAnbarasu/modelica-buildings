...
package CombustionAirNASA
  ...
  extends Modelica.Media.Interfaces.PartialCondensingGases(
     ...
     Temperature(start=T_default, min=200, max=6000));
  ...
  equation
    ...
    assert(noEvent(T >= 200.0), "In "   + getInstanceName() + ": Temperature T exceeded its minimum allowed value of 200 Kelvin)
as required from medium model \"Buildings.Media.CombustionAir\".");
    assert(noEvent(T <= 6000), "In "   + getInstanceName() + ": Temperature T exceeded its maximum allowed value of 6000 Kelvin)
